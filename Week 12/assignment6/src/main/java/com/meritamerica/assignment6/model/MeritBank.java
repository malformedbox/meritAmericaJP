...
public class MeritBank {
	...
	public static CDOffering[] listOfOfferings; //Solution test still requires arrays
	public static List<CDOffering> listOfOfferingsAL;
	...
	static boolean readFromFile(String fileName) {
        try(BufferedReader reader = new BufferedReader(new FileReader(fileName))){
        	...
        	listOfOfferingsAL = new ArrayList<>();
        	...
        	for(int addOffering = 0; addOffering < numCDOfferings; addOffering++) {   		
        		if(listOfOfferings[addOffering] == null) {
        			...
        			listOfOfferingsAL.add(CDOffering.readFromString(dataReadFromFile.get(grabAtIndexFromList)));
        			grabAtIndexFromList++;
        		}
        	}
	...
	static boolean writeToFile(String fileName) {
		...
		try(BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {	
			...
			for(int i = 0; i < listOfAccounts.size(); i++) {
				...
				writer.newLine();
				writer.write(Integer.toString(listOfAccounts.get(i).getListOfCheckingAccounts().size()));
				for(int j = 0; j < listOfAccounts.get(i).getListOfCheckingAccounts().size(); j++) {
					writer.newLine();
					writer.write(listOfAccounts.get(i).getListOfCheckingAccounts().get(j).writeToString());
				}
				writer.newLine();
				writer.write(Integer.toString(listOfAccounts.get(i).getListOfSavingsAccounts().size()));
				for(int j = 0; j < listOfAccounts.get(i).getListOfSavingsAccounts().size(); j++) {
					writer.newLine();
					writer.write(listOfAccounts.get(i).getListOfSavingsAccounts().get(j).writeToString());
				}
				writer.newLine();
				writer.write(Integer.toString(listOfAccounts.get(i).getListOfCDAccounts().size()));
				for(int j = 0; j < listOfAccounts.get(i).getListOfCDAccounts().size(); j++) {
					writer.newLine();
					writer.write(listOfAccounts.get(i).getListOfCDAccounts().get(j).writeToString());
				}
			}
			return true;
		}catch (Exception e) {
